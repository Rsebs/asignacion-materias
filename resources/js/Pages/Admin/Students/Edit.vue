...
const { student, coursesStudent } = defineProps([
	'student',
	'coursesStudent'
]);
...
const cities = ref({});
const courses = ref({});
const document = ref(student.document);
const names = ref(student.names);
const last_names = ref(student.last_names);
const phone = ref(student.phone);
const email = ref(student.email);
const home_address = ref(student.home_address);
const city_id = ref(student.city_id);
const semester = ref(student.semester);
const load = ref(true);
...
async function update() {
	...
	getApi(route('students.update', student.id), 'PUT', data);
			...
			<div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
				<div class="bg-white p-5 overflow-hidden shadow-xl sm:rounded-lg">
					<form @submit.prevent="update">
						<h1 class="text-red-700 text-3xl my-5 uppercase font-bold text-center">Editar estudiante</h1>
						<div>
						...
						<div>
							...
							<TextInput v-model="semester" id="semester" type="number" class="mt-1 block w-full" required
								autocomplete="username" />
						</div>
						<div>
									...
									<tr v-else v-for="c in courses">
										...
										<td class="border border-slate-300 grid grid-cols-3">
											<div v-for="p in c.professors" class="flex justify-between items-center border-b-2 border-r-2 p-1">
												...
												<input :checked="coursesStudent.some(course => course.id === c.id)" type="radio" :id="c.id + '-' + p.id" :name="c.id">
											</div>
										</td>
									</tr>
								</tbody>
